...
(function($){
	...
	$.fn.crop = function(options,userId,headimgurl,userType){
		var v = {
			w:320,
			h:320,
			r:120,
		...
		if(o.res==''){
			...
			//alert("userId:"+userId+"url:"+headimgurl);
                o.res=headimgurl;

			//img\\info\\parent\\2017\\12\\21\\1513845866397.jpg";
		}
		image.src = o.res;
		image.onload = function(){
			...
			if(box.html()!=''){
				...
				box.append('\
					...
					<div id="cut-img" style="width:'+o.w+'px;height:'+o.h+'px;position:relative;top:'+(-o.h)+'px;left:0px;background-image:url('+image.src+');background-size:'+o.r*2+'px '+o.r*2+'px;background-position:0px 0px;background-repeat:no-repeat;"></div><canvas id="temp-img" style="display:none;"></canvas>\
				'); //background-size:'+o.r*2+'px '+o.r*2+'px 初始化为方框大小
				//设置盒子样式
				box.css({
					"width":o.w+"px",
					"height":o.h+"px",
				...
				//box.after('<div id="title_text" style="position:absolute;top:5px;left:0px;width:100%;line-height:40px;color:#fff;text-align:center;z-index:12;">设置头像</div> <div class="row col-sm-12" style="height:'+v.barHeight+'px;width:100%;position:absolute;bottom:0px;"> <div class="col-sm-4" style="display: inline;text-align: center"><a class="btn btn-default" id="back_btn">返回</a></div> <div class="col-sm-4" style="display: inline;text-align: center"><a class="btn btn-default" id="select_btn">选择</a></div> <div class="col-sm-4" style="display: inline;text-align: center"><a class="btn btn-default" id="edit_finished">完成</a></div></div><input type="file" id="selectImageFile" accept="image/*" capture="camera" style="height:0px;width:0px;border:0px;display:none;" />');
                box.after('<div id="title_text" style="position:absolute;top:5px;left:0px;width:100%;line-height:40px;color:#fff;text-align:center;z-index:12;">设置头像</div>' +
					'<div style="height:'+v.barHeight+'px;width:100%;position:absolute;bottom:0px;">' +
					'<a style="display:block;position:absolute;left:15px;bottom:15px;color:#fff;z-index:12;background:#f89119;width:25%;height:30px;line-height:30px;text-align:center;border-radius:3px;" id="back_btn">返回</a>' +
                    '<a style="display:block;position:absolute;left:35%;right:35%;bottom:15px;color:#fff;z-index:12;background:#f89119;width:30%;height:30px;line-height:30px;text-align:center;border-radius:3px;" id="select_btn">选择</a>' +
					'<a style="display:block;position:absolute;right:15px;bottom:15px;color:#fff;z-index:12;background:#35b345;width:25%;height:30px;line-height:30px;text-align:center;border-radius:3px;" id="edit_finished">完成</a><input type="file" id="selectImageFile" accept="image/*" capture="camera" style="height:0px;width:0px;border:0px;display:none;" /></div>');
			...
			if(first){
				...
				$('#select_btn').click(function(){
                    ...
                    if (isIos) {
                        $('#selectImageFile').removeAttr("capture");
                    };
			...
			function uploadIcon(){
                ...
                formData.append("img",convertBase64UrlToBlob(canvas.toDataURL()));
                $.ajax({
                    type: "POST",
                    url:"/wx/"+userType+"Center/"+userId+"/modifyIcon",
                    data: formData,
                    //下面的一定要加
                    // 告诉jQuery不要去处理发送的数据
                    processData : false,
