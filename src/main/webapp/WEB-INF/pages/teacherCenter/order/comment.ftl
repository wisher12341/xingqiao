...
<body>
        ...
        <div class="start" align="center">
            <input disabled="disabled" id="input-22a" type="number" class="rating" min=0 max=5 step=1 data-size="xl"  value="${comment.level}">
        </div>


        <div style="margin-top:20px;vertical-align: middle;font-size: 40px;" align="center">
            <textarea style="width:95%;height:50%;border: 1px solid black" readonly >${comment.detail}</textarea>
        </div>
        ...
        <div style="font-size: 50px;margin-top: 20px" align="center">
            ...
            <#else>
                <input type="radio" name="isOpen"  id="anonymi" value="0" style="width:50px;height:50px;margin: 13px" checked>匿名
            </#if>
        </div>
        <div>
                <#list ((comment.picUrls)?split('#'))! as pic>
                    <img src="${path}/${pic}" onclick="picAdd(this)" width="200px">
                </#list>
        </div>
        <#if (comment.teacherComment.id)??>
        <div>
            <p class="teacher_reply">治疗师回应</p>
            <p class="teacher_reply_detail">${comment.teacherComment.detail}</p>
        </div>
        <#else>
        <p class="teacher_reply">治疗师未回应</p>
        <form action="${path}/wx/teacherCenter/order/comment/addReply" method="post">
            ...
            <div style="width:500px;line-height: 40px;vertical-align: middle;font-size: 16px;">
                <textarea style="height: 150px;width: 500px" placeholder="请填写您的评价" id="comment_t" name="reply" ></textarea>
            </div>
            <div class="reply_btn">
                <button class="btn btn-primary">回应</button>
            </div>
        </form>
        </#if>
